...
plot_dotted_reliabilitydiag <- function(y,
  ...
  ggplot() +
    ...
    autolayer(
      ...
      type = "miscalibration",
      params_histogram = NA,
      params_ribbon = list(fill = fill_colour,
                           alpha = fill_alpha),
      params_CEPsegment = NA,
      params_CEPline = list(size = .5,
                            colour = cep_colour),
      params_CEPpoint = list(size = cep_point_size,
                             colour = cep_colour)
    ) + ylab("CEP") + xlab("Forecast value")
}
...
rooto_discrete <- function(y, yrep, ...) {
  x_range <- range(y, yrep)[1]:range(y, yrep)[2]
  counts_y <- sapply(x_range, function(n) sum(y == n))
  counts_yrep <- apply(
    outer(yrep, x_range, "=="),
    3,
    rowSums)
